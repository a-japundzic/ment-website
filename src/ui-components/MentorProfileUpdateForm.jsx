...
import { getMentorProfile } from "../graphql/queries";
import { updateMentorProfile } from "../graphql/mutations";
...
export default function MentorProfileUpdateForm(props) {
  const {
    id: idProp,
    mentorProfile: mentorProfileModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mentorProfileRecord
      ? { ...initialValues, ...mentorProfileRecord }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getMentorProfile.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMentorProfile
        : mentorProfileModelProp;
      setMentorProfileRecord(record);
    };
    queryData();
  }, [idProp, mentorProfileModelProp]);
  React.useEffect(resetStateValues, [mentorProfileRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateMentorProfile.replaceAll("__typename", ""),
            variables: {
              input: {
                id: mentorProfileRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MentorProfileUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mentorProfileModelProp)}
